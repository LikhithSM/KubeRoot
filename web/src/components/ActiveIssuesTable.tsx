...
function severityClass(confidence: string): string {
  switch (confidence) {
    case 'high':
          ...
          {items.map((issue) => (
            ...
            >
              ...
              <td className="px-6 py-3 text-sm">
                <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${severityClass(issue.diagnosis.confidence)}`}>
                  {issue.diagnosis.confidence.toUpperCase()}
                </span>
              </td>
