...
type CurrentFailuresResponse struct {
	Cluster string                 `json:"cluster"`
	Count   int                    `json:"count"`
	Items   []store.CurrentFailure `json:"items"`
}
...
func (h *Handler) DiagnoseCurrent(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	filter := store.DiagnosisHistoryFilter{Limit: 100}
	if rawLimit := r.URL.Query().Get("limit"); rawLimit != "" {
		parsedLimit, err := strconv.Atoi(rawLimit)
		if err != nil || parsedLimit <= 0 {
			http.Error(w, "invalid limit", http.StatusBadRequest)
	...
	items, err := h.store.ListCurrentFailures(ctx, orgID, clusterID, filter)
	if err != nil {
		http.Error(w, "failed to load current failures: "+err.Error(), http.StatusInternalServerError)
		return
	}

	response := CurrentFailuresResponse{
		Cluster: clusterID,
		Count:   len(items),
		Items:   items,
	}
